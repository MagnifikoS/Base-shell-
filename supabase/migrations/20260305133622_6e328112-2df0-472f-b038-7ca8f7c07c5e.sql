...
-- ═══════════════════════════════════════════════════════════════
-- MODULE: Price Alerts V0.2 — Fail-open + no updated_at on client
-- Fixes: trigger cannot crash the parent UPDATE (fail-open wrapper)
-- Fix D: don't touch updated_at on client product (let optimistic lock handle it)
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ══ FAIL-OPEN WRAPPER ══
  -- The entire sync+alert logic is wrapped in an exception block.
  -- If ANY error occurs (missing tables, constraint violations, etc.),
  -- we catch it and RETURN NEW so the parent UPDATE is never blocked.
  BEGIN
    ...
    LOOP
      -- 1) Sync price on client product — Fix D: do NOT touch updated_at
      --    Let the optimistic lock trigger (trg_products_v2_optimistic_lock) handle it
      UPDATE products_v2
      SET final_unit_price = NEW.final_unit_price
      WHERE id = rec.local_product_id;

      -- 2) Alert creation (inner fail-open: skip on any error)
      BEGIN
        ...
        -- Resolve threshold by category_id (UUID key)
        ...
        -- Insert alert if threshold exceeded
      ...
      EXCEPTION WHEN OTHERS THEN
        -- Alert creation failed — skip silently (fail-open for alerts)
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Outer fail-open: entire sync failed — log and let parent UPDATE succeed
...
COMMENT ON FUNCTION public.fn_sync_b2b_price() IS 'Module Alertes Prix V0.2 — Fail-open: never blocks parent UPDATE. Fixes: anti-boucle, nom_produit, category_id UUID, no updated_at on client. Droppable.';

...
BEGIN
  ...
  -- ═══ 3.5 NEW: Update supplier DRAFT stock_document_lines with validated quantities ═══
  -- This ensures fn_post_stock_document reads the CLIENT-validated quantities,
  -- not the original shipment quantities. Stock events will match the BL exactly.
  ...
  -- 4. POST the supplier's WITHDRAWAL stock document (now with corrected quantities)
  ...
  -- 9. Update order status → CLOSED directly (stock is already correct)

...
-- ═══════════════════════════════════════════════════════════════
-- MODULE: Price Alerts (V0) — Étape 2 : Trigger sync prix B2B
-- Isolation : droppable via DROP TRIGGER + DROP FUNCTION
-- Ne modifie AUCUN module existant
-- ═══════════════════════════════════════════════════════════════

-- Fonction SECURITY DEFINER : sync prix + création alertes
...
BEGIN
  ...
  LOOP
    ...
    IF rec.client_old_price IS NULL OR rec.client_old_price = 0 THEN
      v_variation_pct := 100; -- treat as 100% change if no previous price
    ELSE
    ...
    -- 4) Determine threshold (category override or global)
...
COMMENT ON FUNCTION public.fn_sync_b2b_price() IS 'Module Alertes Prix V0 — sync prix B2B + alertes. Droppable indépendamment.';

-- Trigger sur products_v2
CREATE TRIGGER trg_sync_b2b_price
  AFTER UPDATE OF final_unit_price
  ON public.products_v2
  FOR EACH ROW
  EXECUTE FUNCTION public.fn_sync_b2b_price();

COMMENT ON TRIGGER trg_sync_b2b_price ON public.products_v2 IS 'Module Alertes Prix V0 — déclenche sync prix B2B. Droppable: DROP TRIGGER trg_sync_b2b_price ON products_v2;';

-- ═══════════════════════════════════════════════════════════════════════════
-- Étape 2 : Réordonnancement fn_post_b2b_reception pour atomicité inter-org
-- AVANT : POST fournisseur (STEP 1-2) AVANT validations client (STEP 4-9)
-- APRÈS : Toutes validations client AVANT tout impact fournisseur
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Si une validation échoue → RETURN ok=false, rien n'est touché
  -- ═══════════════════════════════════════════════════════════════════
  ...
  -- ═══ GUARD 6 (NEW — moved BEFORE supplier POST): Validate client zones & snapshots ═══
  -- This was previously STEP 4-9, executed AFTER the supplier withdrawal POST.
  -- If any of these fail, supplier stock would have been decremented with no client receipt.
  ...
  -- Check: all client products must have a storage zone
  ...
  -- Check: all client product zones must have an active snapshot
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
  ...
  -- ═══════════════════════════════════════════════════════════════════
  -- PHASE B : MUTATIONS (toutes les validations ont passé)
  -- À partir d'ici, tout échec est un bug/infra — le EXCEPTION global
  -- rollback toute la transaction PL/pgSQL automatiquement.
  -- ═══════════════════════════════════════════════════════════════════

  -- ═══ STEP 1: Post supplier withdrawal (SKIP if already POSTED at ship time) ═══
  ...
  -- ═══ STEP 7: Generate B2B invoices (non-blocking — failure = warning, not rollback) ═══
  BEGIN
  ...
  -- ═══ STEP 8: TVA France enrichment (non-blocking) ═══
  BEGIN

...
-- ═══════════════════════════════════════════════════════════════════════
-- ÉTAPE 3 : Brancher le catalogue B2B sur la mutualisation
-- 
-- Règles :
-- 1. Produit hors groupe → apparaît normalement
-- 2. Produit carrier d'un groupe COHÉRENT → apparaît avec display_name du groupe
-- 3. Produit membre (non-carrier) d'un groupe COHÉRENT → masqué
-- 4. Produit dans un groupe INCOHÉRENT → chaque membre apparaît individuellement
--    (pas de mutualisation exposée tant que le groupe n'est pas corrigé)
--
-- Cohérence = tous les membres ont la même unit_category sur final_unit_id
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  --
  -- Step A: Identify coherent vs incoherent groups for this supplier
  -- A group is coherent if all members share the same unit category
  --
  -- Step B: For each product, decide visibility:
  --   - Not in any group → include as-is
  --   - Carrier of coherent group → include (with group display_name)
  --   - Non-carrier member of coherent group → EXCLUDE
  --   - Any member of incoherent group → include as-is (fallback safe)

  WITH coherent_groups AS (
    -- Groups where all members have the same unit category
  ...
  product_group_info AS (
    -- For each product, get its group membership info
    SELECT
      imm.product_id,
      img.id AS group_id,
  ...
  visible_products AS (
    ...
    WHERE p.establishment_id = v_supplier_est_id
      ...
      AND (
        -- Case A: not in any group → visible
        pgi.product_id IS NULL
        -- Case B: carrier of coherent group → visible
        OR (pgi.is_coherent AND pgi.is_carrier)
        -- Case C: incoherent group → all members visible (safe fallback)
  ...
  FROM (
    SELECT
      p.id,
      -- Use group display_name for carrier of coherent group, else product name
      CASE
  ...
  -- 4. Get units used by supplier products (unchanged)
...
COMMENT ON FUNCTION fn_get_b2b_catalogue(UUID, UUID) IS
'Catalogue B2B avec mutualisation. '
'Produits hors groupe: visibles normalement. '
'Carrier de groupe cohérent: visible avec display_name du groupe. '
'Membres non-carrier de groupe cohérent: masqués. '
'Groupes incohérents: tous les membres visibles individuellement (fallback safe).';

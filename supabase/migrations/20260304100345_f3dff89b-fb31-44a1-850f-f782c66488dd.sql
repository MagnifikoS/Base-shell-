...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM _ship_lines) THEN
    ...
    LOOP
      -- AUTO-BOOTSTRAP snapshot if missing (include organization_id)
      IF NOT EXISTS (
        ...
        INSERT INTO zone_stock_snapshots (
          ...
          v_commande.supplier_establishment_id, v_org_id, v_zone_id, gen_random_uuid(), p_user_id
        );
        ...
        SELECT
          (SELECT snapshot_version_id FROM zone_stock_snapshots
           WHERE establishment_id = v_commande.supplier_establishment_id AND storage_zone_id = v_zone_id),
          sl.supplier_product_id, 0, sl.canonical_unit_id, 0, 'INIT_AFTER_SNAPSHOT'

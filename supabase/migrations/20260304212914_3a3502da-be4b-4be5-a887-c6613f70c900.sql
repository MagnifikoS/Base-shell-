...
-- ═══════════════════════════════════════════════════════════════════════════
-- FIX R-02: fn_receive_commande — reject reception if any product has no zone
--
-- Strategy: Option A — refuse with explicit error listing affected products
-- This prevents silent stock loss when a product has no storage_zone_id
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ═══ 1b. FIX R-02: Check all received products have a storage zone ═══
  ...
  WHERE cl.commande_id = p_commande_id
    AND cp.storage_zone_id IS NULL;
  ...
  -- ═══ 3. Determine reception type + detect écarts ═══
  FOR v_line IN
  ...
  -- ═══ 4. Create litige if écarts detected (ATOMIC with status change) ═══
  IF v_has_ecarts THEN
    ...
    FROM commande_lines cl
    WHERE cl.commande_id = p_commande_id
      AND cl.received_quantity IS NOT NULL
      AND cl.shipped_quantity IS NOT NULL
      AND cl.received_quantity != cl.shipped_quantity;

    -- Set commande to 'litige' (not 'recue')
    UPDATE commandes
    SET status = 'litige',
  ...
  -- ═══ 5. Stock ledger — RECEIPT to client (always, based on received qty) ═══

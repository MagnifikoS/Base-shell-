...
-- ═══════════════════════════════════════════════════════════════════════════
-- TVA France V2: Dedicated RPC + 3 P0 fixes
--
-- Fix 1: issued → bl_withdrawal_lines (supplier products)
--         received → bl_app_lines (client products)  
-- Fix 2: Rebuild-safe idempotency (DELETE + reinsert, vat_enriched_at flag)
-- Fix 3: Real feature toggle via vat_france_b2b_enabled setting
-- Architecture: separate fn, called from fn_post_b2b_reception (1 line)
-- ═══════════════════════════════════════════════════════════════════════════

-- 1. Add vat_enriched_at to invoices for idempotency flag
ALTER TABLE invoices ADD COLUMN IF NOT EXISTS vat_enriched_at timestamptz DEFAULT NULL;

-- 2. Create the dedicated VAT enrichment function
...
BEGIN
  -- ═══ Feature toggle: check env setting ═══
  -- Set via: ALTER DATABASE postgres SET app.vat_france_b2b_enabled = 'true';
  -- Or per-transaction: SET LOCAL app.vat_france_b2b_enabled = 'true';
  ...
  -- ═══ Process each B2B invoice for this order ═══
  FOR v_inv IN
  ...
  LOOP
    -- Idempotency: already enriched → skip
    IF v_inv.vat_enriched_at IS NOT NULL THEN
      CONTINUE;
    END IF;

    -- Rebuild-safe: delete any partial lines from a previous failed attempt
    ...
    -- ═══ SOURCE ROUTING ═══
    -- issued  → bl_withdrawal_lines (supplier products from BL Retrait)
    -- received → bl_app_lines (client products from BL Réception)

    IF v_inv.b2b_status = 'issued' THEN
      -- ISSUED invoice: use BL Retrait lines (supplier-side products)
      FOR v_src_line IN
        SELECT
          bwl.product_id,
      ...
      LOOP
        ...
        ELSIF v_src_line.cat_lower IN (
          ...
          v_vat_rate := 0.20;
        ELSE
          v_warnings := COALESCE(v_warnings, '') ||
            'UNKNOWN_CATEGORY:' || COALESCE(v_src_line.cat_lower, 'NULL') ||
            ':product:' || v_src_line.product_id::text || ';';
        END IF;
    ...
    ELSIF v_inv.b2b_status = 'received' THEN
      -- RECEIVED invoice: use BL App lines (client-side products from BL Réception)
      -- Find the BL reception document for this order
      FOR v_src_line IN
        SELECT
          bal.product_id,
...
-- 3. Update fn_post_b2b_reception: replace inline STEP 12 with function call
...
BEGIN
  ...
  -- ═══ STEP 4-9: Stock receipt + BL reception (unchanged) ═══
  ...
  -- ═══ STEP 12: TVA France enrichment (single function call) ═══
  -- Supprimable: drop fn_enrich_b2b_invoices_vat_fr + drop b2b_invoice_lines + remove this call
  BEGIN

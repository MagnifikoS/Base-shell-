-- ═══════════════════════════════════════════════════════════════════════════
-- FIX: Merge 'commande' (singulier) → 'commandes' (pluriel)
-- The frontend uses 'commandes', RPC notification uses 'commande'
-- Strategy: take MAX(access_level) between the two for each role, then delete orphan
-- ═══════════════════════════════════════════════════════════════════════════

-- Step 1: Update 'commandes' permissions where 'commande' has a higher access level
-- Administrateur: commande=full > commandes=write → update to full
UPDATE role_permissions 
SET access_level = 'full', scope = 'org'
WHERE module_key = 'commandes' 
  AND role_id = '00000000-0000-0000-0000-000000000001';

-- Salarié: commande=read/establishment > commandes=none/self → update to read/establishment
UPDATE role_permissions 
SET access_level = 'read', scope = 'establishment'
WHERE module_key = 'commandes' 
  AND role_id = '00000000-0000-0000-0000-000000000004';

-- Step 2: Delete all 'commande' (singulier) permissions — now merged into 'commandes'
DELETE FROM role_permissions WHERE module_key = 'commande';

-- Step 3: Delete the orphan module entry
DELETE FROM modules WHERE key = 'commande';

-- Step 4: Update fn_send_commande_notification to use 'commandes' (pluriel)
...
BEGIN
  ...
  -- ═══ FIXED: use 'commandes' (pluriel) to match frontend module key ═══
  FOR v_recipient IN

...
-- Drop old function signature first (required because parameter defaults changed)
DROP FUNCTION IF EXISTS public.fn_post_stock_document(UUID, INT, TEXT, UUID, TEXT, BOOLEAN, TEXT);

-- ═══════════════════════════════════════════════════════════════════════════
-- FIX P0: Route stock_events by product zone (not document zone)
-- Multi-zone safe: snapshot + negative check per product zone
-- Blocage dur si produit sans zone
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT
    ...
    p.storage_zone_id,          -- ← PRODUCT zone
    dl.product_id,
    p_document_id,
    ...
    dl.canonical_label,
    dl.context_hash,
    zss.snapshot_version_id,    -- ← Snapshot of PRODUCT zone
    p_override_flag,
    p_override_reason,
    p_posted_by
  FROM stock_document_lines dl

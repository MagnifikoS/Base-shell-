...
-- Fix: add RECURSIVE keyword to the allocation CTE
...
BEGIN
  ...
  member_stock AS (
    SELECT gmf.*,
      GREATEST(COALESCE(il.quantity, 0) + COALESCE(SUM(se.delta_quantity_canonical), 0), 0) AS available_stock
    FROM group_members_filtered gmf
    LEFT JOIN zone_stock_snapshots zss
      ON zss.storage_zone_id = gmf.member_zone_id AND zss.establishment_id = v_commande.supplier_establishment_id
    LEFT JOIN inventory_lines il
      ON il.session_id = zss.snapshot_version_id AND il.product_id = gmf.member_product_id
    LEFT JOIN stock_events se
      ON se.product_id = gmf.member_product_id AND se.storage_zone_id = gmf.member_zone_id
      AND se.snapshot_version_id = zss.snapshot_version_id AND se.canonical_family = gmf.supplier_family
    GROUP BY gmf.line_id, gmf.client_shipped_qty, gmf.total_supplier_qty,
             ...
             gmf.member_zone_id, gmf.member_product_name, il.quantity
  ),
  ranked_members AS (

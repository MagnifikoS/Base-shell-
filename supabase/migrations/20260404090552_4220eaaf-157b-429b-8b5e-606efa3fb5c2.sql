...
BEGIN
  -- ═══ STEP 0: Validate commande ═══
  SELECT c.*, c.supplier_establishment_id AS supplier_est_id
  INTO v_order
  FROM commandes c
  WHERE c.id = p_order_id;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Commande % not found', p_order_id;
  END IF;

  IF v_order.status NOT IN ('expediee') THEN
    RAISE EXCEPTION 'Commande % has invalid status for reception: %', p_order_id, v_order.status;
  END IF;
  ...
  -- ═══ NOTE: Supplier-side withdrawal is already handled by fn_ship_commande ═══
  -- Steps 1+2 (legacy supplier stock posting + bl_withdrawal_lines) removed.
  -- fn_ship_commande creates stock documents, posts them, and writes commande_lines.shipped_quantity.
  ...
  LOOP
    -- ═══ Strict canonical_family validation ═══
  ...
  -- ═══ STEP 7: Update commande status → recue ═══
  UPDATE commandes
  SET
    status = 'recue',
    received_at = now(),
    received_by = p_client_user_id::text,
    reception_type = 'b2b_inter_establishment',
    updated_at = now()
  WHERE id = p_order_id
    AND status NOT IN ('recue', 'cloturee');
  ...
  -- ═══ STEP 10: Auto-generate invoice supplier link ═══
  BEGIN
    SELECT

...
-- ═══════════════════════════════════════════════════════════════════════
-- ÉTAPE 4 : Import B2B = carrier uniquement
--
-- Ajoute un garde-fou dans fn_import_b2b_product_atomic :
-- Si le source_product_id est un membre (non-carrier) d'un groupe
-- cohérent, on le redirige automatiquement vers le carrier_product_id.
-- Si le groupe est incohérent, l'import passe tel quel (pas de mutualisation).
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Check if the source product is in a mutualization group
  SELECT img.carrier_product_id,
         -- Coherent = all members share the same unit category
         (COUNT(DISTINCT mu.category) OVER (PARTITION BY img.id)) <= 1
  INTO v_carrier_id, v_is_coherent
  ...
  IF v_carrier_id IS NOT NULL AND v_is_coherent THEN
    -- Coherent group: force to carrier
    v_effective_source_product_id := v_carrier_id;
  END IF;
  -- Incoherent group or no group: keep original source_product_id

...
-- ═══════════════════════════════════════════════════════════════════════
-- Rule 0: shipped_quantity can NEVER exceed canonical_quantity
-- Source of truth: DB trigger on commande_lines
-- ═══════════════════════════════════════════════════════════════════════

-- 1. Trigger function: clamp shipped_quantity to canonical_quantity
...
-- 2. Attach trigger BEFORE INSERT OR UPDATE
DROP TRIGGER IF EXISTS trg_commande_lines_clamp_shipped ON public.commande_lines;
CREATE TRIGGER trg_commande_lines_clamp_shipped
  BEFORE INSERT OR UPDATE ON public.commande_lines
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_clamp_shipped_quantity();

-- 3. Update fn_ship_commande: add inline clamp + return clamped flag
  ...
  IF EXISTS (SELECT 1 FROM _ship_lines) THEN
    ...
    LOOP
      ...
      INSERT INTO stock_documents (
        ...
        'WITHDRAWAL', 'DRAFT', p_user_id, v_idemp_key || ':' || v_zone_id::text, p_commande_id
      );
      ...
      SELECT public.fn_post_stock_document(
        ...
        p_event_reason := 'B2B_SHIPMENT', p_override_flag := true,

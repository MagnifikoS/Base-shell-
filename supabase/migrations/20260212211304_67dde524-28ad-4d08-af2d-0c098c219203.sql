...
-- Drop both overloads and recreate with proper event_reason handling + line completeness guards

DROP FUNCTION IF EXISTS public.fn_post_stock_document(uuid, integer, uuid, text, boolean, text, text);
DROP FUNCTION IF EXISTS public.fn_post_stock_document(uuid, integer, uuid, text, boolean, text);
...
BEGIN
  ...
  -- ═══ 6. Line completeness guard (prevent NOT NULL crashes) ═══
  ...
  IF v_incomplete_count > 0 THEN
    RAISE EXCEPTION 'LINE_INCOMPLETE_CANONICAL_DATA: % ligne(s) avec données canoniques manquantes (unit/family/hash/delta).', v_incomplete_count;
  END IF;
  ...
  -- ═══ 9. Negative stock check (after lock, before events) ═══
  ...
  -- ═══ 10. INSERT stock_events (v_event_reason guaranteed NOT NULL) ═══

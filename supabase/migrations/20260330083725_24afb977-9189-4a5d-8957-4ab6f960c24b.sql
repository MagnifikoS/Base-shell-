...
BEGIN
  ...
  -- Step 0b: If in a group, check coherence separately
  ...
  -- C4: Ensure stock_handling_unit_id is never null
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    -- BUG-3 FIX: Check init result in UPDATE path too (key = 'ok')
  ...
  ELSE
    ...
    -- BUG-2 FIX: was checking 'success' but fn_initialize_product_stock returns 'ok'
    v_init_result := fn_initialize_product_stock(v_product_id, p_user_id, 0);
    IF NOT COALESCE((v_init_result ->> 'ok')::BOOLEAN, false) THEN
      RAISE EXCEPTION 'STOCK_INIT_FAILED:%', COALESCE(v_init_result ->> 'error', 'unknown');
    END IF;

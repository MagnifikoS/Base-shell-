...
-- ═══════════════════════════════════════════════════════════════════════════
-- fn_product_unit_price_factor: Returns the multiplication factor to convert
-- a price FROM one unit TO another for a given product.
-- Uses packaging levels (conditionnement_config) + universal unit_conversions.
    ...
    FOR v_idx IN 1..array_length(v_queue_units, 1) LOOP
      ...
      LOOP
        IF rec.from_unit_id = v_current_unit AND NOT rec.to_unit_id = ANY(v_visited) THEN
          v_next_factor := v_current_factor * rec.factor;

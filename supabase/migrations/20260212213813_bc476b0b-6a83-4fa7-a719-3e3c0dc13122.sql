...
-- ═══════════════════════════════════════════════════════════════════════════
-- ESTABLISHMENT STOCK SETTINGS — default receipt zone
-- ═══════════════════════════════════════════════════════════════════════════

CREATE TABLE public.establishment_stock_settings (
  establishment_id UUID NOT NULL PRIMARY KEY REFERENCES public.establishments(id) ON DELETE CASCADE,
  default_receipt_zone_id UUID REFERENCES public.storage_zones(id) ON DELETE SET NULL,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_by UUID
);

-- Enable RLS
ALTER TABLE public.establishment_stock_settings ENABLE ROW LEVEL SECURITY;

-- RLS policies
CREATE POLICY "Users can view stock settings for their establishments"
ON public.establishment_stock_settings FOR SELECT
USING (establishment_id IN (SELECT public.get_user_establishment_ids()));

CREATE POLICY "Users can update stock settings for their establishments"
ON public.establishment_stock_settings FOR UPDATE
USING (establishment_id IN (SELECT public.get_user_establishment_ids()));

CREATE POLICY "Users can insert stock settings for their establishments"
ON public.establishment_stock_settings FOR INSERT
WITH CHECK (establishment_id IN (SELECT public.get_user_establishment_ids()));

-- Seed: for each establishment with zones, pick "ZONE DE STOCKAGE" if exists, else first zone
INSERT INTO public.establishment_stock_settings (establishment_id, default_receipt_zone_id)
SELECT DISTINCT ON (e.id)
  e.id,
  COALESCE(
    (SELECT sz.id FROM storage_zones sz WHERE sz.establishment_id = e.id AND sz.name = 'ZONE DE STOCKAGE' LIMIT 1),
    (SELECT sz.id FROM storage_zones sz WHERE sz.establishment_id = e.id ORDER BY sz.created_at LIMIT 1)
  )
FROM establishments e
WHERE EXISTS (SELECT 1 FROM storage_zones sz WHERE sz.establishment_id = e.id)
ON CONFLICT (establishment_id) DO NOTHING;

-- ═══════════════════════════════════════════════════════════════════════════
-- UPDATE fn_post_stock_document — auto-resolve zone for RECEIPT
-- ═══════════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_event_reason TEXT;
  v_effective_zone_id UUID;
  v_default_receipt_zone_id UUID;
BEGIN
  -- ═══ 0. Fetch document ═══
  ...
  -- ═══ 4. Resolve effective zone ═══
  IF v_doc.type = 'RECEIPT' THEN
    -- For receipts, ALWAYS use the establishment's default receipt zone
    SELECT default_receipt_zone_id INTO v_default_receipt_zone_id
    FROM establishment_stock_settings
    WHERE establishment_id = v_doc.establishment_id;

    IF v_default_receipt_zone_id IS NULL THEN
      RETURN jsonb_build_object('ok', false, 'error', 'DEFAULT_RECEIPT_ZONE_MISSING',
        'message', 'La zone de réception par défaut n''est pas configurée.');
    END IF;

    v_effective_zone_id := v_default_receipt_zone_id;

    -- Update the document's zone to match (for traceability)
    UPDATE stock_documents SET storage_zone_id = v_effective_zone_id, updated_at = now()
    WHERE id = p_document_id;
  ELSE
    v_effective_zone_id := v_doc.storage_zone_id;
  END IF;

  -- ═══ 5. Snapshot must exist for effective zone ═══
  SELECT * INTO v_snapshot FROM zone_stock_snapshots
  WHERE establishment_id = v_doc.establishment_id
    AND storage_zone_id = v_effective_zone_id;

  IF v_snapshot IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'NO_ACTIVE_SNAPSHOT', 
      'zone_id', v_effective_zone_id);
  END IF;
  ...
  -- ═══ 9. ATOMIC UPDATE with lock_version check ═══
  ...
  IF p_override_flag = false THEN
    WITH current_estimates AS (
      ...
      LEFT JOIN (
        ...
        FROM stock_events
        WHERE storage_zone_id = v_effective_zone_id
  ...
  -- ═══ 11. INSERT stock_events (using effective zone) ═══
  ...
  RETURN jsonb_build_object(
    ...
    'effective_zone_id', v_effective_zone_id,

...
BEGIN
  ...
  member_stock AS (
    SELECT gmf.*,
      GREATEST(
        COALESCE(il.quantity, 0) + COALESCE(ev.total_delta, 0),
        0
      ) AS available_stock
  ...
  -- ═══ 3b. SUPPLIER TRUTH DEFICIT RECOVERY ═══
  -- If total allocated < supplier_quantity (stock was insufficient across all members),
  -- assign the deficit to the carrier product. fn_post_stock_document will clamp
  -- the actual stock event to available stock (stock floor = 0, never negative).
  INSERT INTO _allocation_lines (line_id, member_product_id, member_zone_id, member_product_name,
    supplier_unit_id, supplier_family, conversion_factor, allocated_qty)
  SELECT
    sl.line_id,
    sl.carrier_product_id,
    sl.carrier_zone_id,
    sl.supplier_product_name,
    sl.supplier_unit_id,
    sl.supplier_family,
    sl.conversion_factor,
    sl.supplier_quantity - COALESCE(alloc.total_allocated, 0) AS deficit_qty
  FROM _ship_lines sl
  LEFT JOIN (
    SELECT line_id, SUM(allocated_qty) AS total_allocated
    FROM _allocation_lines
    GROUP BY line_id
  ) alloc ON alloc.line_id = sl.line_id
  WHERE sl.conversion_status != 'error'
    AND sl.client_shipped_qty > 0
    AND sl.supplier_quantity > COALESCE(alloc.total_allocated, 0);

  -- 4. Per-zone stock documents
  -- ═══ FIX: Aggregate allocation lines by (member_product_id, member_zone_id) ═══
  -- Before this fix, allocation (step 3) and deficit recovery (step 3b) could each
  -- produce a separate row for the same product (e.g. carrier gets partial allocation
  -- + deficit remainder). Inserting both violated UNIQUE(document_id, product_id).
  -- We now SUM allocated_qty per product before inserting into stock_document_lines.
  CREATE TEMP TABLE _aggregated_allocation ON COMMIT DROP AS
  SELECT
    al.line_id,
    al.member_product_id,
    al.member_zone_id,
    al.member_product_name,
    al.supplier_unit_id,
    al.supplier_family,
    al.conversion_factor,
    SUM(al.allocated_qty) AS allocated_qty
  FROM _allocation_lines al
  GROUP BY al.line_id, al.member_product_id, al.member_zone_id, al.member_product_name,
    al.supplier_unit_id, al.supplier_family, al.conversion_factor;

  FOR v_zone_id IN SELECT DISTINCT member_zone_id FROM _aggregated_allocation WHERE allocated_qty > 0 LOOP
    IF NOT EXISTS (
      ...
      FROM _aggregated_allocation al WHERE al.member_zone_id = v_zone_id ON CONFLICT DO NOTHING;
    END IF;
    ...
    FROM _aggregated_allocation al
  ...
  SET
    shipped_quantity = sl.client_shipped_qty,
    line_status = CASE
      WHEN sl.client_shipped_qty = 0 THEN 'rupture'
      WHEN sl.client_shipped_qty < sl.ordered_qty THEN 'modifie'
      ELSE 'ok'
    END
  FROM _ship_lines sl

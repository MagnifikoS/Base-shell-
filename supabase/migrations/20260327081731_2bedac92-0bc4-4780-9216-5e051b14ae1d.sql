...
-- ═══════════════════════════════════════════════════════════════════
-- Wire fn_log_conversion_error into fn_ship_commande (after step 2b)
-- and fn_resolve_litige (after building _litige_adj_lines)
-- Non-blocking: just logs, never stops the flow.
-- ═══════════════════════════════════════════════════════════════════

-- Add logging to fn_ship_commande: after counting conversion errors,
-- log each one to brain_events for monitoring
...
BEGIN
  ...
  -- ═══ 2b-LOG. Log conversion errors to brain_events ═══
  IF v_conversion_error_count > 0 THEN
    FOR v_err_line IN 
      SELECT supplier_product_id, client_unit_id 
      FROM _ship_lines WHERE conversion_status = 'error'
    LOOP
      PERFORM fn_log_conversion_error(
        v_commande.supplier_establishment_id,
        v_err_line.supplier_product_id,

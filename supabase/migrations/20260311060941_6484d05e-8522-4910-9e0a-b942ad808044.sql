-- ═══════════════════════════════════════════════════════════════════════════
-- FIX: Per-product zone routing in fn_post_stock_document
-- 
-- PROBLEM: The active version (20260228) uses v_doc.storage_zone_id (document header)
-- for snapshot lookup, negative check, and event insertion. This is wrong because
-- products can be in different zones than the document header.
--
-- FIX: Join products_v2.storage_zone_id for each line to:
--   1. Find the correct zone_stock_snapshot per product zone
--   2. Validate negative stock per product zone
--   3. Insert stock_events with the product's real zone
--
-- PRESERVES: Same signature, FOR UPDATE locks, idempotency, override logic
-- REF: STK-04 audit, P0 bug fix
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ═══ 6. Per-product snapshot validation ═══
  -- Lock all relevant zone_stock_snapshots (FOR UPDATE) to prevent concurrent modifications.
  -- Each line's snapshot is determined by the PRODUCT's storage_zone_id, not the document header.
  PERFORM zss.id
  ...
  -- Each event uses the product's storage_zone_id and the corresponding snapshot_version_id

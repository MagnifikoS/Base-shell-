...
-- ═══════════════════════════════════════════════════════════════════════════
-- FIX P0-SSOT: Filter stock_events by snapshot_version_id in negative check
--
-- BUG: The ev_sum subquery sums ALL historical stock_events without filtering
-- by the active snapshot_version_id, violating the SSOT formula:
--   Stock = snapshot_qty + Σ(events WHERE snapshot_version_id = current_snapshot)
--
-- FIX: Add snapshot_version_id filter to ev_sum subquery using a LATERAL join
-- that correlates with line_with_snapshot.snapshot_version_id.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ═══ 9. Negative stock check — PER PRODUCT ZONE — SSOT COMPLIANT ═══
  ...
  IF p_override_flag = false THEN
    ...
    -- ═══ FIX P0-SSOT: Collect active snapshot_version_ids to filter events ═══
    active_snapshots AS (
    ...
    -- Sum only events matching the ACTIVE snapshot (SSOT formula)
    event_sums AS (

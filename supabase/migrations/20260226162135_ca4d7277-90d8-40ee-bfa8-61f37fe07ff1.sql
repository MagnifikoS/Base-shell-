...
CREATE OR REPLACE FUNCTION fn_post_b2b_reception(
  p_order_id UUID,
...
BEGIN
  ...
  INSERT INTO stock_documents (
    establishment_id, organization_id, document_type, status,
    reason, zone_id, created_by, idempotency_key
  ) VALUES (
    p_client_establishment_id, p_client_organization_id, 'RECEIPT', 'DRAFT',
    'INTER_ESTABLISHMENT_TRANSFER', v_client_header_zone_id, p_client_user_id,
    'b2b-receipt-' || p_order_id::text
  ...
  LOOP
    INSERT INTO stock_document_lines (
      ...
      canonical_unit_id, canonical_family, context_hash, zone_id
    ) VALUES (
      v_receipt_doc_id,
      (v_line->>'client_product_id')::UUID,
      ABS((v_line->>'quantity_canonical')::NUMERIC),
      (v_line->>'client_canonical_unit_id')::UUID,
      COALESCE(v_line->>'canonical_family', 'count'),
      v_line->>'context_hash',
      COALESCE(
        (SELECT p.storage_zone_id FROM products_v2 p WHERE p.id = (v_line->>'client_product_id')::UUID),
        v_client_header_zone_id
      )
    );
  END LOOP;
  ...
  v_post_result := fn_post_stock_document(
    ...
    p_idempotency_key := 'b2b-receipt-post-' || p_order_id::text,
    p_posted_by := p_client_user_id,
    p_event_reason := 'INTER_ESTABLISHMENT_TRANSFER',
    p_override_flag := false,
    p_override_reason := NULL
  ...
  SELECT COUNT(*) INTO v_receipt_event_count
  FROM stock_events WHERE document_id = v_receipt_doc_id;
  ...
  BEGIN
    ...
    ELSE
      ...
      -- Client's invoice_suppliers record for the supplier
      ...
      -- Supplier's invoice_suppliers record for the client

...
AS $function$
BEGIN
  -- 1. Auth: caller must belong to the client establishment
  ...
  -- 2. Partnership guard: active relationship required
  IF NOT EXISTS (
  ...
  -- 3. Return ALL non-archived products from the supplier (full catalog)
  RETURN QUERY
  SELECT
    p.id,
    p.nom_produit,
  ...
  WHERE p.establishment_id = p_supplier_establishment_id

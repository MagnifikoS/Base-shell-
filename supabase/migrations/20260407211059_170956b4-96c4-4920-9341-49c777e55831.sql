-- fn_create_product_complete — Atomic product + input config creation (PR-14)
-- Eliminates the P0 atomicity gap between products_v2 INSERT and product_input_config INSERT.
...
DECLARE
  ...
  v_existing_id UUID;
BEGIN
  -- 1. Collision check (name + supplier + establishment)
  ...
  -- 3. INSERT product_input_config (atomique)
  ...
  -- 4. Initialiser le stock si quantité initiale fournie
  IF p_initial_stock_quantity IS NOT NULL
     AND p_initial_stock_quantity > 0
     AND p_storage_zone_id IS NOT NULL THEN
    PERFORM fn_initialize_product_stock(v_product_id, p_user_id);
  END IF;

...
-- Fix: the created_via must be 'INIT_AFTER_SNAPSHOT' to pass the terminated session guard
...
BEGIN
  ...
  IF v_snapshot_record IS NULL THEN
    -- Create bootstrap inventory session
    ...
    INSERT INTO zone_stock_snapshots (
      ...
      p_establishment_id, v_org_id, p_storage_zone_id,
      v_session_id, now(), p_user_id
    );
  ELSE
    v_session_id := v_snapshot_record.snapshot_version_id;
  END IF;

  -- Insert inventory_line qty=0 (INIT_AFTER_SNAPSHOT to pass terminated session guard)

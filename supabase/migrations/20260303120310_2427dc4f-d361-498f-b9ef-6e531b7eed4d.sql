...
-- ═══════════════════════════════════════════════════════════════════════════
-- Phase 1: Add category_id UUID to products_v2 + backfill from text
-- Dual-write: fn_save_product_wizard writes BOTH category + category_id
-- ═══════════════════════════════════════════════════════════════════════════

-- 1. Add column (nullable, no FK constraint yet to allow safe backfill)
ALTER TABLE public.products_v2 
ADD COLUMN IF NOT EXISTS category_id UUID REFERENCES public.product_categories(id);

-- 2. Backfill: resolve category text → category_id for active establishments only
UPDATE public.products_v2 p
SET category_id = pc.id
FROM public.product_categories pc
WHERE pc.establishment_id = p.establishment_id
...
-- 3. Index for performance (filters + JOINs)
CREATE INDEX IF NOT EXISTS idx_products_v2_category_id ON public.products_v2(category_id);

-- 4. Update fn_save_product_wizard to accept category_id and dual-write
CREATE OR REPLACE FUNCTION public.fn_save_product_wizard(
  ...
  -- NEW: category_id UUID (dual-write)
...
BEGIN
  ...
  IF p_category_id IS NOT NULL THEN
    ...
    -- Resolve name from SSOT for backward compat
    SELECT name INTO v_resolved_category_name
    FROM product_categories WHERE id = p_category_id;
  ELSE
    -- Legacy path: resolve from text (backward compat during migration)
  ...
  SET
    ...
    -- DUAL-WRITE: both columns stay in sync
  ...
  RETURN jsonb_build_object(
    ...
    'fields_applied', jsonb_build_array(
      ...
      'category', 'category_id', 'storage_zone_id'
    )
  );

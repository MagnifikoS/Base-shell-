...
-- ═══════════════════════════════════════════════════════════════════════════
-- FIX P0: fn_post_b2b_reception — Bypass DRAFT state for client RECEIPT
-- 
...
-- Impact: ZERO on intra-org AMIR flows. fn_post_stock_document unchanged.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ═══ STEP 5: Validate snapshot exists for client zone ═══
  SELECT * INTO v_snapshot FROM zone_stock_snapshots
  WHERE establishment_id = p_client_establishment_id
    AND storage_zone_id = v_client_header_zone_id;

  IF v_snapshot IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'NO_CLIENT_SNAPSHOT', 'zone_id', v_client_header_zone_id);
  END IF;

  -- ═══ STEP 6: Create client RECEIPT directly as POSTED (bypass DRAFT) ═══
  -- This avoids uq_stock_documents_one_draft_per_zone_type entirely
  ...
  -- ═══ STEP 8: Create stock_events directly (inline — replaces fn_post_stock_document for RECEIPT) ═══

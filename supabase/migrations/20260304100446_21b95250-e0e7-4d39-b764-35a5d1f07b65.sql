...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM _ship_lines) THEN
    ...
    LOOP
      -- AUTO-BOOTSTRAP: create inventory_session + snapshot + lines if missing
      IF NOT EXISTS (
        ...
        -- Create a bootstrap inventory session (status=termine)
        ...
        -- Create snapshot pointing to this session
        ...
        -- Bootstrap inventory_lines at qty=0

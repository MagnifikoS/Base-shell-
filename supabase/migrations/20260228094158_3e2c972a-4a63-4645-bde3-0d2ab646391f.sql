...
-- FIX: Replace NEW.document_type → NEW.type (column name error from Étape 6)
-- Only change: 2 occurrences of document_type → type. No other logic change.
...
BEGIN
  ...
  IF NEW.status = 'POSTED' AND OLD.status = 'DRAFT'
     AND NEW.type IN ('ADJUSTMENT', 'RETURN') THEN
...
DROP FUNCTION IF EXISTS public._test_post_trigger_error(uuid);

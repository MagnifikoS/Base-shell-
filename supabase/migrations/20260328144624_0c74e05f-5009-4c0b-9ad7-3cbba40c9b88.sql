...
-- Fix fn_ship_commande: replace broken DISTINCT window function with GROUP BY
-- and simplify recursive allocation to a simpler iterative approach
...
BEGIN
  ...
  IF v_conversion_error_count > 0 THEN
    FOR v_err_line IN SELECT * FROM _ship_lines WHERE conversion_status = 'error' LOOP
      ...
      VALUES (
        ...
        'b2b_conversion_error',
        'fn_ship_commande',
        jsonb_build_object(
          'commande_id', p_commande_id,
          'line_id', v_err_line.line_id,
          'product_id', v_err_line.carrier_product_id,
          'client_unit_id', v_err_line.client_unit_id,
          'context', 'shipment'
        )
      );
    END LOOP;
  ...
  -- FIX: Use GROUP BY instead of window function (COUNT DISTINCT OVER not supported)
  group_coherence AS (
    SELECT
      gmr.group_id,
      COUNT(DISTINCT mu.category) <= 1 AS is_coherent
  ...
  -- Sequential allocation: use a recursive CTE for clean iteration
  allocation_recursive AS (
    -- Base case: rank 1
    SELECT
      rm.line_id, rm.member_product_id, rm.member_zone_id, rm.member_product_name,
    ...
    -- Recursive step: rank N uses remaining from rank N-1
    SELECT
      rm.line_id, rm.member_product_id, rm.member_zone_id, rm.member_product_name,
      rm.supplier_unit_id, rm.supplier_family, rm.conversion_factor,
      rm.rn, rm.available_stock, rm.total_supplier_qty,
  ...
  SELECT
    ar.line_id,
    ar.member_product_id,
    ar.member_zone_id,
    ar.member_product_name,
    ar.supplier_unit_id,
    ar.supplier_family,
    ar.conversion_factor,
    ar.allocated AS allocated_qty
  FROM allocation_recursive ar;

  -- ═══ 4. Per-zone stock documents ═══
  FOR v_zone_id IN
    SELECT DISTINCT member_zone_id FROM _allocation_lines WHERE allocated_qty > 0
  LOOP
    -- ── 4a. Bootstrap snapshot if needed ──
    IF NOT EXISTS (
      SELECT 1 FROM zone_stock_snapshots
      WHERE storage_zone_id = v_zone_id
        AND establishment_id = v_commande.supplier_establishment_id
    ) THEN

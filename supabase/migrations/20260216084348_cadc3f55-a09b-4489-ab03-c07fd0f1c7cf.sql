...
CREATE OR REPLACE FUNCTION public.fn_post_stock_document(p_document_id uuid, p_expected_lock_version integer, p_idempotency_key text DEFAULT NULL::text, p_posted_by uuid DEFAULT NULL::uuid, p_event_reason text DEFAULT NULL::text, p_override_flag boolean DEFAULT false, p_override_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ═══ 6. Snapshot validation — ALL types use document's zone ═══
  ...
  -- ═══ 6b. RECEIPT: warn if products belong to different zones ═══
  IF v_doc.type = 'RECEIPT' THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'message', 'Produit "' || p.nom_produit || '" est normalement en zone ' || COALESCE(sz.name, p.storage_zone_id::text) || ', réception postée en zone du document.'
    )), '[]'::jsonb)
    INTO v_warnings
  ...
  -- ═══ 9. Negative stock check (ALL types: single zone from document) ═══
  ...
  -- ═══ 10. INSERT stock_events — ALL types use document's zone + snapshot ═══

...
-- ═══════════════════════════════════════════════════════════════
-- 1. Insert missing modules into the modules table
-- ═══════════════════════════════════════════════════════════════

INSERT INTO modules (key, name, display_order) VALUES
  ('bl_app', 'Bons de livraison', 116),
  ('stock_ledger', 'Journal de stock', 117),
  ('stock_alerts', 'Alertes de stock', 118),
  ('vision_ai', 'Scan factures IA', 119),
  ('dlc_critique', 'DLC critique', 120)
ON CONFLICT (key) DO NOTHING;

-- ═══════════════════════════════════════════════════════════════
-- 2. Update wizard RPC to seed conversions + settings
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Seed measurement units from templates
  ...
  -- ═══ NEW: Seed unit conversions (g↔kg, ml↔L, cl↔L, cl↔ml) ═══
  WITH conversion_pairs(from_abbr, to_abbr, factor) AS (
    VALUES
      ('g',  'kg', 0.001::numeric),
      ('kg', 'g',  1000::numeric),
      ('ml', 'L',  0.001::numeric),
  ...
  SELECT v_est_id, mu_from.id, mu_to.id, cp.factor
  FROM conversion_pairs cp
  JOIN measurement_units mu_from ON mu_from.establishment_id = v_est_id AND mu_from.abbreviation = cp.from_abbr
  JOIN measurement_units mu_to   ON mu_to.establishment_id   = v_est_id AND mu_to.abbreviation   = cp.to_abbr
  ON CONFLICT DO NOTHING;

  GET DIAGNOSTICS v_conversions_seeded = ROW_COUNT;

  -- Seed default storage zones
  ...
  -- ═══ NEW: Seed extraction_settings (OCR IA defaults) ═══
  INSERT INTO extraction_settings (establishment_id, organization_id)
  VALUES (v_est_id, v_org_id)
  ON CONFLICT (establishment_id) DO NOTHING;

  -- ═══ NEW: Seed dlc_alert_settings (DLC defaults) ═══
  INSERT INTO dlc_alert_settings (establishment_id)
  VALUES (v_est_id)
  ON CONFLICT (establishment_id) DO NOTHING;

  -- ═══ NEW: Seed establishment_stock_settings (reception zone) ═══
  SELECT id INTO v_default_zone_id
  FROM storage_zones
  WHERE establishment_id = v_est_id
  ORDER BY display_order
  LIMIT 1;

  INSERT INTO establishment_stock_settings (establishment_id, default_receipt_zone_id)
  VALUES (v_est_id, v_default_zone_id)
  ON CONFLICT (establishment_id) DO NOTHING;
  ...
  RETURN jsonb_build_object(
    ...
    'conversions_seeded', v_conversions_seeded,

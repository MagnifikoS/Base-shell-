...
-- Fix fn_cancel_b2b_shipment: skip documents with 0 events (rupture/clamp scenario)
...
BEGIN
  ...
  FOR v_doc IN
    ...
    ORDER BY created_at
  LOOP
    -- Skip documents with no events (rupture/clamp to 0 scenario)
    SELECT count(*) INTO v_event_count
    FROM stock_events WHERE document_id = v_doc.id AND event_type != 'VOID';
    
    IF v_event_count = 0 THEN
      -- Mark as VOID directly (no stock to reverse)
      UPDATE stock_documents
      SET status = 'VOID', voided_by = p_user_id, voided_at = now(),
          lock_version = lock_version + 1, updated_at = now()
      WHERE id = v_doc.id AND status = 'POSTED';
  ...
  RETURN jsonb_build_object(
    ...
    'skipped_empty_documents', v_skipped_count,

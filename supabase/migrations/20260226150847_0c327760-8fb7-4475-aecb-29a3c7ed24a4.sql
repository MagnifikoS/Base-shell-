...
-- Add B2B invoice columns to invoices table
ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS b2b_order_id UUID REFERENCES public.product_orders(id),
  ADD COLUMN IF NOT EXISTS b2b_status TEXT CHECK (b2b_status IN ('issued', 'received'));

-- Index for idempotency check and filtering
CREATE INDEX IF NOT EXISTS idx_invoices_b2b_order_id ON public.invoices (b2b_order_id) WHERE b2b_order_id IS NOT NULL;

-- Extend fn_post_b2b_reception to generate dual invoices
...
BEGIN
  ...
  -- ═══ 11. NEW: Generate B2B invoices (supplier issued + client received) ═══
  ...
  BEGIN
    ...
    ELSE
      ...
      -- Find supplier's own invoice_suppliers record (self-supplier) - not needed, use establishment directly
      -- Find client's invoice_suppliers record for this B2B partner
      ...
      -- Find supplier's own self-reference supplier record (for issued invoice)
      -- The supplier needs a "self" supplier record or we use any matching one
      ...
      -- 11a. Supplier invoice (ISSUED) — only if supplier has a supplier record for the client
      ...
      -- 11b. Client invoice (RECEIVED) — only if client has a supplier record for the partner
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Invoice generation failed but reception succeeded — log warning

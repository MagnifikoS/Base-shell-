...
BEGIN
  ...
  -- 2. Build _ship_lines
  -- client_shipped_qty = supplier validated amount, capped at ordered_qty
  ...
  -- V1 SUPPLIER TRUTH: allocation spreads across members by available stock,
  -- but any deficit (stock insufficient) is re-assigned to carrier so full
  -- supplier_quantity always flows to stock documents. fn_post_stock_document
  -- handles the physical floor (stock never negative).
  ...
  allocation_recursive AS (
    -- Base case: first member (most stock)
  ...
  -- BUSINESS RULE: Supplier validation = truth for shipped_quantity
  -- The supplier confirms what was physically sent. Stock is adjusted independently.
  -- If stock < validated qty, stock goes to 0 (never negative) but client sees full validated amount.

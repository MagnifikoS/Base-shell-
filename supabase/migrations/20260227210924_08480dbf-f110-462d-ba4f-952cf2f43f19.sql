...
-- ═══════════════════════════════════════════════════════════════════════════
-- FIX: fn_post_b2b_reception — Accept POSTED supplier stock document
-- 
-- Root cause: Ship now POSTs the withdrawal immediately (no persistent DRAFT).
-- GUARD 5 must accept POSTED status and skip supplier-side steps 1-2.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ═══ STEP 1-2: Post supplier withdrawal (SKIP if already POSTED at ship time) ═══
  IF NOT v_supplier_already_posted THEN
    -- Legacy path: update draft lines then post
  ...
  -- ═══ STEP 4-9: Stock receipt + BL reception ═══

...
-- ═══════════════════════════════════════════════════════════════════
-- Fix: fn_resolve_litige — override ciblé surplus uniquement
-- ═══════════════════════════════════════════════════════════════════
-- Problème: Phase 3 a mis p_override_flag := false, ce qui bloque
-- la résolution de litiges surplus quand le stock fournisseur est
-- insuffisant (NEGATIVE_STOCK exception).
--
-- Correction: override conditionnel — true UNIQUEMENT si la zone
-- contient au moins une ligne surplus (delta_sign = -1).
-- Les litiges manquant-only restent override=false (Stock Zéro V1).
-- ═══════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ══════════════════════════════════════════════════════════════════════
  -- Build temp table WITH B2B conversion (same pattern as fn_ship_commande)
  -- delta = shipped - received:
  --   positive → manquant (stock returns to supplier)
  --   negative → surplus  (stock removed from supplier)
  -- ══════════════════════════════════════════════════════════════════════
  ...
  -- ── Remove lines with conversion errors ──
  ...
  LOOP
    ...
    -- ══════════════════════════════════════════════════════════════════════
    -- Detect if this zone has surplus lines (delta_sign = -1)
    -- Surplus = marchandise déjà partie physiquement → correction comptable
    -- Override autorisé UNIQUEMENT dans ce cas
    -- ══════════════════════════════════════════════════════════════════════
    SELECT EXISTS (
      SELECT 1 FROM _litige_adj_lines
      WHERE supplier_zone_id = v_zone_id AND delta_sign = -1
    ) INTO v_zone_has_surplus;
    ...
    -- ══════════════════════════════════════════════════════════════════════
    -- Write stock_document_lines with SUPPLIER unit & converted quantity
    -- delta_sign preserves direction: +1 manquant (return), -1 surplus (withdraw)
    -- ══════════════════════════════════════════════════════════════════════
    ...
    -- ══════════════════════════════════════════════════════════════════════
    -- Post document:
    --   surplus present → override=true (correction comptable, stock déjà parti)
    --   manquant only  → override=false (Stock Zéro V1 respecté)
    -- ══════════════════════════════════════════════════════════════════════
    SELECT public.fn_post_stock_document(
      ...
      p_override_reason := CASE WHEN v_zone_has_surplus
        THEN 'Réconciliation litige surplus — marchandise déjà expédiée physiquement'
        ELSE NULL END
    ) INTO v_post_result;

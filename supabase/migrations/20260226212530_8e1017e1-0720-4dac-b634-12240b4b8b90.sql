...
-- ═══════════════════════════════════════════════════════════════════════════
-- HARD FIX: PR4 + Anti-silence — fn_post_b2b_reception
-- 
-- Changes vs current version:
-- 1. PR4: Replace ON CONFLICT(establishment_id, name) with explicit
--    partner_establishment_id lookup (no name-based fusion risk)
-- 2. Anti-silence: Remove EXCEPTION WHEN OTHERS on invoice block.
--    Invoice errors now surface in the RPC return with details
--    instead of being swallowed. The main transaction still completes
--    because we use a separate flag approach.
-- 3. Log invoice warnings to audit_logs for observability.
-- 
-- AMIR non impacté — this function is B2B-only.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Cache establishment names to avoid repeated subqueries
  ...
  -- PR4 FIX: Use partner_establishment_id lookup, NOT ON CONFLICT(name)
  -- ANTI-SILENCE FIX: No more EXCEPTION WHEN OTHERS — errors propagate
  -- but wrapped in a savepoint so invoice failure doesn't kill the whole tx
  BEGIN
    IF EXISTS (SELECT 1 FROM invoices WHERE b2b_order_id = p_order_id LIMIT 1) THEN
      v_invoice_warning := NULL; -- already exists, skip
    ELSE
      ...
      -- ═══ PR4: Lookup by partner_establishment_id ONLY ═══
      -- Client-side supplier record (who supplied to client)
      ...
      -- If not found and partnership exists, create (by ID, not by name)
      ...
      -- Supplier-side supplier record (who the client is as a "supplier" for issued invoices)
      ...
      -- Insert supplier-side "issued" invoice
      ...
      -- Insert client-side "received" invoice
  ...
  EXCEPTION WHEN OTHERS THEN
    -- ANTI-SILENCE: Log the error to audit_logs instead of swallowing

...
-- ═══════════════════════════════════════════════════════════════════════════
-- P0-3: fn_post_stock_document — per-product zone for RECEIPT
-- ═══════════════════════════════════════════════════════════════════════════
-- RECEIPT events use products_v2.storage_zone_id (per line), NOT default zone.
-- Each event gets the snapshot_version_id from its product's zone.
-- WITHDRAWAL/ADJUSTMENT still use the document's storage_zone_id (single zone).
...
BEGIN
  ...
  -- ═══ 6. RECEIPT: check all products have storage_zone_id ═══
  IF v_doc.type = 'RECEIPT' THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object('product_id', dl.product_id)), '[]'::jsonb)
    ...
    IF v_missing_zone_products != '[]'::jsonb THEN
      RETURN jsonb_build_object('ok', false, 'error', 'PRODUCT_NO_ZONE',
        'message', 'Un ou plusieurs produits n''ont pas de zone de stockage assignée.',
        'products', v_missing_zone_products);
    END IF;

    -- Check all concerned zones have a snapshot
    SELECT COALESCE(jsonb_agg(DISTINCT p.storage_zone_id), '[]'::jsonb)
    ...
    IF v_missing_snapshot_zones != '[]'::jsonb THEN
      RETURN jsonb_build_object('ok', false, 'error', 'NO_ACTIVE_SNAPSHOT',
        'message', 'Aucun inventaire de référence pour certaines zones.',
        'zones', v_missing_snapshot_zones);
    END IF;
  ELSE
    -- WITHDRAWAL/ADJUSTMENT: single zone from document
  ...
  IF p_override_flag = false THEN
    IF v_doc.type = 'RECEIPT' THEN
      -- Per-product zone negative check for RECEIPT
      WITH current_estimates AS (
        ...
        LEFT JOIN (
          ...
          JOIN zone_stock_snapshots z2 
            ON z2.establishment_id = v_doc.establishment_id 
            AND z2.storage_zone_id = se.storage_zone_id
          WHERE se.snapshot_version_id = z2.snapshot_version_id
          GROUP BY se.product_id, se.storage_zone_id
        ) ev_sum ON ev_sum.product_id = dl.product_id AND ev_sum.storage_zone_id = p.storage_zone_id
        WHERE dl.document_id = p_document_id
      ),
      negatives AS (
        SELECT 
          product_id, snapshot_qty, events_delta, line_delta,
      ...
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object('product_id', product_id, 'current_estimated', ROUND((snapshot_qty + events_delta)::numeric, 4),
          'delta', line_delta, 'resulting_stock', resulting_stock)
      ), '[]'::jsonb) INTO v_negative_products FROM negatives;
    ELSE
      -- Single zone negative check for WITHDRAWAL/ADJUSTMENT
      WITH current_estimates AS (
        SELECT
          dl.product_id,
          dl.delta_quantity_canonical AS line_delta,
  ...
  IF v_doc.type = 'RECEIPT' THEN
    -- RECEIPT: per-product zone + per-zone snapshot
    ...
    WHERE dl.document_id = p_document_id;
  ELSE
    -- WITHDRAWAL/ADJUSTMENT: single zone
    INSERT INTO stock_events (
      establishment_id, organization_id, storage_zone_id, product_id,
      document_id, event_type, event_reason,
      delta_quantity_canonical, canonical_unit_id, canonical_family, canonical_label,

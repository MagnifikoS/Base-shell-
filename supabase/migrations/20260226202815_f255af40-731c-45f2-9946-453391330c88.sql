...
BEGIN
  ...
  -- 4. Update supplier draft lines with validated quantities
  ...
  -- 6. Rebuild BL withdrawal lines with validated quantities
  ...
  -- 8. Create client RECEIPT stock document (CORRECT column names)
  ...
  -- 9. Create client stock document lines (NO zone_id column — routing handled by fn_post)
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_validated_lines)
  LOOP
  ...
  -- 10c. Close order and update received quantities
  ...
  -- 11. Generate B2B invoices (in exception block — non-blocking)
  BEGIN
